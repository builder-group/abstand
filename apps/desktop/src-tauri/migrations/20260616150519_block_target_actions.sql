...
-- Create "new_intention_block_app_target" table
CREATE TABLE `new_intention_block_app_target` (
...
-- Copy rows from old table "intention_block_app_target" to new temporary table "new_intention_block_app_target"
INSERT INTO `new_intention_block_app_target` (`intention_id`, `app_id`, `created_at`) SELECT `intention_id`, `app_id`, `created_at` FROM `intention_block_app_target`;
-- Drop "intention_block_app_target" table after copying rows
DROP TABLE `intention_block_app_target`;
-- Rename temporary table "new_intention_block_app_target" to "intention_block_app_target"
ALTER TABLE `new_intention_block_app_target` RENAME TO `intention_block_app_target`;
...
INSERT INTO `new_intention_block_website_target` (`intention_id`, `website_id`, `created_at`) SELECT `intention_id`, `website_id`, `created_at` FROM `intention_block_website_target`;
...
-- Manual backfill: rows under allow_targets scope represent allowed targets, not blocked targets.
UPDATE intention_block_app_target
SET action = 'allow'
WHERE intention_id IN (
    SELECT intention_id
    FROM intention_block
    WHERE scope = 'allow_targets'
);

UPDATE intention_block_website_target
SET action = 'allow'
WHERE intention_id IN (
    SELECT intention_id
    FROM intention_block
