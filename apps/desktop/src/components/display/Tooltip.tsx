		...
		<TooltipPrimitive.Root disableHoverablePopup={disableHoverablePopup} {...rest}>
			...
			<TooltipPrimitive.Portal>
				<TooltipPrimitive.Positioner side={side} sideOffset={6} className="z-50">
					...
					>
						{content}
						{shortcut != null && <Kbd size="sm">{shortcut}</Kbd>}
					</TooltipPrimitive.Popup>
...
const tooltipVariants = cva(
	'bg-base-950/90 text-base-50 border-base-50/10 inline-flex items-center rounded-lg border font-medium shadow-md origin-(--transform-origin) transition-[opacity,scale] duration-150 ease-out data-starting-style:scale-95 data-starting-style:opacity-0 data-ending-style:scale-95 data-ending-style:opacity-0',
	{
		variants: {
			size: {
				default: 'gap-2 px-2.5 py-1.5 text-xs',
