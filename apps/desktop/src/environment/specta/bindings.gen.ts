...
async isAccessibilityPermissionGranted() : Promise<boolean> {
    return await TAURI_INVOKE("is_accessibility_permission_granted");
},
async openAccessibilityPermissionSettings() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("open_accessibility_permission_settings") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
